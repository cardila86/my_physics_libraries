...
class plottingTools:
    ...
    def _read_bands_vaspkit_projected(self, path_read, orbs, fermi_vaspkit=False, klabels_bool=False, kticks_bool=False):
        '''
        Reads the band structure generated by vaspkit1.3.5. It
        works for both spin and spinless calculations.
        ...
        if os.path.isfile(path_read):
            data = np.loadtxt(path_read, skiprows=1)
            data = data.transpose()
            kpoints = data[0]
            bands = data[1]
    ...
    def plot_bands_projected(self,
        ...
        n = 0
        for orb in orbs:
            if type(orb)!=float or type(orb)!=int:
                orbs_grouped = True
                n += 1 
                for i in orb:
                    orbs_read.append(i)
        ...
        if n > 3:
            print('ERROR: In this moment, orbs input supports only 3 orbitals (grouped orbitals count as 1).')

        path_read += '/' + root
        if klabels is None and kticks is None:
            kpoints, E, klabels, orbs_projection_list, kticks = self._read_bands_vaspkit(path_read, orbs_read,fermi_vaspkit=E_vaspkit, klabels_bool=True, kticks_bool=True)
        elif klabels is None and kticks is not None:
            kpoints, E, orbs_projection_list, klabels = self._read_bands_vaspkit(path_read, orbs_read, fermi_vaspkit=E_vaspkit, klabels_bool=True, kticks_bool=False)
        elif klabels is not None and kticks is None:
            kpoints, E, orbs_projection_list, kticks = self._read_bands_vaspkit(path_read, orbs_read, fermi_vaspkit=E_vaspkit, klabels_bool=False, kticks_bool=True)
        else:
            kpoints, E = self._read_bands_vaspkit(path_read, orbs_read, fermi_vaspkit=E_vaspkit, klabels_bool=False, kticks_bool=False)
        # ------------ ax, fig objects -----------
        if ax is None and kbreaks is None:
            fig, ax = plt.subplots()
        elif ax is None and kbreaks is not None:
