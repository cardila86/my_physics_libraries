...
#
#   Last Update
#    A.Romero. 05.12.2014  Improve st. Now plots intensities
#
...
Implemented Classes:
 ...
 -UtilsProcar: handy methods, not intented to be used directly by the user
...
class ProcarParser:
  ...
  def __init__(self, loglevel=logging.WARNING):
    ...
    self.kpoints = None      
    # Number of kpoints, as given by the KPOINTS header
    ...
    # spin. 
    # NOTE: before calling to `readOrbitals` the case '4' is marked as '1'
  ...
  def readOrbital(self):
    ...
    Undefined behavior in case of phase factors if LORBIT 12.
...
class ProcarFileFilter:
  ...
  -To group the atoms 1,2,3,4 and 5,6,7,8 from PROCAR and write them
   ...
   -To select just the magnetization vector from PROCAR and write it in
    POTCAR-new:
  ...
  def FilterBands(self, Min, Max):
    ...
    # open the files
    fout = open(self.outfile, 'w')
    fout1 = open(self.outfile+".band", 'w')
    fopener = UtilsProcar()
    fin = fopener.OpenFile(self.infile)
  ...
  def FilterSpin(self, components):
    ...
    # open the files
    fout = open(self.outfile, 'w')
    fout1 = open(self.outfile+'.Spins', 'w')
    fopener = UtilsProcar()
    with fopener.OpenFile(self.infile) as fin:
      counter = 0
      for line in fin:
        # if any data found 
        if re.match(r"\s*k-point", line):
          self.kpoints = re.findall(r"k-point\s+\d+\s*:\s+([-.\d\s]+)", line)
          fout1.write("\n %s  " % self.kpoints[0])
...
class ProcarPlot:
  ...
  def plotBands(self, ax=None, size=None, marker='o', ticks=None):
    # ------------ ax, fig objects -----------
    if ax is None:
      fig, ax = plt.subplots()
    else:
    ...
    ax.plot(xaxis,self.bands.transpose(), 'r-', marker=marker, 
                    markersize=size)
    ...
    if ticks:
      ...
      plt.set_xticks(ticks, ticksNames)

    return fig, ax

  def scatterPlot(self, ax=None, size=50, mask=None, cmap='hot_r', vmax=None, vmin=None,
                  marker='o', ticks=None):
    # ------------ ax, fig objects -----------
    if ax is None:
      fig, ax = plt.subplots()
    ...
    if mask is not None:
      ...
      mbands = self.bands
    ax.scatter(xaxis, mbands, c=self.spd, s=size, linewidths=0,
  ...
  def parametricPlot(self, ax, cmap='hot_r', vmin=None, vmax=None, mask=None, 
                     ticks=None):
    ...
    for y,z in zip(mbands,self.spd):
     ...
     # print points
      ...
      lc = LineCollection(segments, cmap=plt.get_cmap(cmap), norm=norm, 
                          alpha=0.8)
      lc.set_array(z)
      lc.set_linewidth(1) #ACG-change the linewidth in the bands
      matplotlib.rcParams.update({'font.size': 20, 'font.family': 'serif'})
  ...
  def atomicPlot(self, ax=None, cmap='hot_r', vmin=None, vmax=None):
    """
...
#st AHR
  def st(self, spin, noarrow, sx, sy, sz, interpolation=200):
    ...
    #selecting components of K-points-ACG multiply by a factor the axis
    xold,yold = 5.01*self.kcart[:,0], 5.01*self.kcart[:,1]
    font = {'family' : 'serif',
            'weight' : 'normal',
	    'size'   : 20}
...
#    ymax=0.025
    self.log.debug(      "xlim = " + str([xmin, xmax]) 
                    +  "  ylim = " + str([ymin, ymax]))
    if (noarrow):
      interpolation=interpolation*30
    xnew, ynew = np.mgrid[xmin:xmax:interpolation*1j, 
                          ymin:ymax:interpolation*1j]
    ...
    if (not noarrow): 
       ...
       cont = [plt.contour(xnew, ynew, z, [self.energy], linewidths=0.2,
                        colors='k', linestyles='solid') for z in bnew]
   # plt.axis("equal")
    ...
    for (contour, spinX, spinY, spinZ) in zip(cont, sx, sy, sz):
      ...
      if (not noarrow): 
        ...
        if (spin == 1):
           plt.scatter(points[::6,0], points[::6,1], c=newSx[::6], s=50,edgecolor='none',alpha=1.0,marker=".",cmap='seismic',norm=colors.normalize(-1.0,1.0))
           plt.xlim(-0.5, 0.5)
           plt.ylim(-0.5, 0.5)
        elif (spin == 2):
           plt.scatter(points[::6,0], points[::6,1], c=newSy[::6], s=50,edgecolor='none',alpha=1.0,marker=".",cmap='seismic',norm=colors.normalize(-1.0,1.0))
           plt.xlim(-0.5, 0.5)
           plt.ylim(-0.5, 0.5)
        elif (spin == 3):
           plt.scatter(points[::6,0], points[::6,1], c=newSz[::6], s=50,edgecolor='none',alpha=1.0,marker=".",cmap='seismic',norm=colors.normalize(-1.0,1.0))
           plt.xlim(-0.5, 0.5)
           plt.ylim(-0.5, 0.5)
    plt.colorbar()
    plt.grid(True)
...
def scriptCat(args):
  ...
  if args.gz and args.outFile[-3:] != '.gz':
    args.outFile += '.gz'
    if args.quiet == False:
...
def scriptBandsplot(args):
  ...
  if args.verbose:
    ...
    if args.permissive and args.quiet == False:
...
# como incluir esto en el menu, por ahora solo en el menu
  ...
  # Plotting the data
  print("In the following line I substract the fermi energy for all bands = ",args.fermi)
...
def scriptFermi2D(args):
  ...
  if args.quiet == False:
    ...
    print("outcar         : ", args.outcar)
    print("st             : ", args.st)
    print("arrows/intensity : ", args.noarrow)
...
##  fs.SetInversion(args.inversion)
  if not args.st:
    fs.Plot(mask=args.mask, interpolation=600)
  else:
    fs.st(args.spin, args.noarrow, sx=stData[0], sy=stData[1], sz=stData[2])
...
if __name__ == "__main__":
  ...
  phelp = ("Plot of the spin texture with arrows or intensity for a given spin direction on the Fermi's surface."
           " This option works quite indepentent of another options but needs to set st and spin.")
